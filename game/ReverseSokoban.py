...
from deadlock_detection.reverse_detect_deadlocks import check_deadlock
...
class ReverseSokobanBoard:
    ...
    def construct(self, level, player, steps):
        new_board = ReverseSokobanBoard(level_id=self.level_id, pre=self.pre, max_steps=self.max_steps, deadlocks=self.deadlocks)
