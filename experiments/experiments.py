...
parser.add_argument('--num_sims', default=1000, type=int, help='Number of simulations in the MCTS')
...
for level_id in tqdm(range(NUM_LEVELS)):
    ...
    outcome = solver.solve(level_id+1, args.folder, args.num_sims, args.max_steps, args.verbose, args.mode)
    outcomes[level_id] = 1 if outcome == "WIN" else 0
print(f"Soleved {sum(outcomes)} out of {NUM_LEVELS} levels with {args.num_sims} Simulations.")